...
## 使用方法
1. 本示例通过 `EXTRA_COMPONENT_DIRS` 使用仓库根目录中的本地 epdiy 组件，不需要再把旧版 epdiyv2.0 复制到 `components` 文件夹。
...
4. ESP32-S3 修改版硬件使用 `epd_board_epdiy2_s3`，GPIO46 驱动电压开关已封装在 board definition 中，不再需要在示例里宏定义 `epd_poweron()` / `epd_poweroff()`。
